resource "aws_instance" "C4_MongoDB1_West" {
  ...
  provisioner "chef" {
    ...
    node_name       = "C4_MongoDB1_West_${self.id}"
...
resource "aws_instance" "C4_MongoDB2_West" {
  # Custom AMI for Mongo Instances
  ami                    = "${lookup(var.Mongo_AMIs, var.primary_region)}"
  instance_type          = "r4.xlarge"
  subnet_id              = "${aws_subnet.Dev_mongo_west.id}"
  ...
  provisioner "chef" {
    environment     = "c1"
    run_list        = ["mongodb::default"]
    node_name       = "C4_MongoDB2_West_${self.id}"
    secret_key      = "${file("./dbsec")}"
    server_url      = "https://api.chef.io/organizations/REDACTED"
    user_name       = "terraform_validator"
    recreate_client = false
...
resource "aws_instance" "C4_MongoDB3_West" {
  # Custom AMI for Mongo Instances
  ami                    = "${lookup(var.Mongo_AMIs, var.primary_region)}"
  instance_type          = "r4.xlarge"
  subnet_id              = "${aws_subnet.Dev_mongo_west.id}"
  ...
  provisioner "chef" {
    environment     = "c1"
    run_list        = ["mongodb::default"]
    node_name       = "C4_MongoDB3_West_${self.id}"
    secret_key      = "${file("./dbsec")}"
    server_url      = "https://api.chef.io/organizations/REDACTED"
    user_name       = "terraform_validator"
    recreate_client = false
...
resource "null_resource" "configure_mongo_replicaset_and_backup" {
  triggers {
    replicaset_instance_ids = "${aws_instance.C4_MongoDB1_West.id},${aws_instance.C4_MongoDB2_West.id},${aws_instance.C4_MongoDB3_West.id}"
  }
  ...
  provisioner "chef" {
    attributes_json = <<-EOF
        {
            "mongodb": {
                "replicaset_members": {
                    "0": "${aws_instance.C4_MongoDB1_West.private_ip}",
                    "1": "${aws_instance.C4_MongoDB2_West.private_ip}"
                },
                "replicaset_member_low_priority": {
                    "2": "${aws_instance.C4_MongoDB3_West.private_ip}"
                }
            }
        }
    EOF
    ...
    node_name       = "C4_MongoDB1_West_${aws_instance.C4_MongoDB1_West.id}"
    secret_key      = "${file("./dbsec")}"
    server_url      = "https://api.chef.io/organizations/REDACTED"
    user_name       = "terraform_validator"
    recreate_client = false
    ...
    connection {
      type = "ssh"
      user = "centos"
      host = "${aws_instance.C4_MongoDB1_West.public_ip}"
  ...
  provisioner "chef" {
    ...
    node_name       = "C4_MongoDB3_West_${aws_instance.C4_MongoDB3_West.id}"
    secret_key      = "${file("./dbsec")}"
    server_url      = "https://api.chef.io/organizations/REDACTED"
    user_name       = "terraform_validator"
    recreate_client = false
    ...
    connection {
      type = "ssh"
      user = "centos"
      host = "${aws_instance.C4_MongoDB3_West.public_ip}"
